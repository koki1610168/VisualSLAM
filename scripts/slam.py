...
if __name__ == "__main__":
    ...
    while cap.isOpened():
        ...
        good = []
        for p1, p2 in points:
            a1, a2 = map(lambda x: int(round(x)), p1.pt)
            b1, b2 = map(lambda x: int(round(x)), p2.pt)
            if abs(a1 - b1) <= 30 and abs(a2 - b2) <= 30:
                good.append(((a1, b1), (a2, b2)))
                cv2.circle(frame, (a1, a2), color=(255, 0, 0), radius=3)
                cv2.line(frame, (a1, a2), (b1, b2), color=(0, 255, 0))
